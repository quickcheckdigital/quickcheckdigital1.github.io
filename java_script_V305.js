// ----------------------------------------------
// Stand 22.11.2022 version 3.04 (veroeffentlicht)
// script fuer QuickCheck Fragebogen
// Erweiterung um 5 weitere Kundendimensionen (von 18 auf 23)
// Erweiterung um weiter 3 dummy Fragen - Version V305 mit 15 Fragen
// statisches App

// ### INITIALIZATION 
// ----------------------------------------------
// ### VARIABLES
// Anzahl der Fragen ifrag=14
// Anzahl der Kundenschnittstellen idig=23
//-----------------------------------------------
// Die Anzahl der aktiven Fragen muss aktiv geaendert werden und 
// auch die Fragetexte eingegeben werden
// Dazu muessen die jeweils auskommentierten Zeilen
// aktviert werden.
// bei der Aktivierung zusuetzllciher Fragen ist der HTLM Code
// ebenfalls anzupassen, damit die zusaetzlichen Fragen
// auch gedruckt werden.
// User variables (+1 wegen der Form der Abfrage)
ifrag=15
idig=23+1
//-----------------------------------------------
// Function to download data to a file nachtraeglich eingefuehrt
...
var sum_max=[]
var ergebnis=[]
// Bewertungsmatrix vorbereitet für 23 Fragen, die letzten drei Fragen sind dummy-Fragen
// und werden deshalb nicht bewertet
bewertung=[
...
function initialize_page() { 
	// Initialize buttons
	// button1: Fragebogen zuruecksetzten -> mode="default"
    ...
    document.getElementById("F15A").checked=false
	document.getElementById("F15B").checked=false
	document.getElementById("F15C").checked=false
	document.getElementById("F15D").checked=false
	//
    //document.getElementById("F16A").checked=false
	//document.getElementById("F16B").checked=false
	//document.getElementById("F16C").checked=false
	//document.getElementById("F16D").checked=false
	
    //document.getElementById("F17A").checked=false
	//document.getElementById("F17B").checked=false
	//document.getElementById("F17C").checked=false
	//document.getElementById("F17D").checked=false
    //
...
function count() {
    ...
    j=0
	document.getElementById('check').value="Bitte die 15 Fragen beantworten"
	document.getElementById('check1').value="Bitte die 15 Fragen beantworten"
	for (var i = 1; i <= ifrag; i++){
			antwort[i]=1
    }
    // set default color
    document.getElementById("Q1").style.backgroundColor="floralwhite"
	...
	// $('html,body').animate({scrollTop: $(target)},'slow');
        ...
        for (i=0;i<4;i++){
            if (document.getElementsByName("F15")[i].checked==true) {
                antwort[15]=i}
            }

            //for (i=0;i<4;i++){
            //    if (document.getElementsByName("F16")[i].checked==true) {
            //        antwort[16]=i}
            //}
            //for (i=0;i<4;i++){
            //    if (document.getElementsByName("F17")[i].checked==true) {
            //        antwort[17]=i}
            //}
            //
        ...
        var ergebnis_matrix = new Array(idig);

        for (var i = 0; i < idig-1; i++) {
        ...
        // Sortierung der Rangfolge (Groesse) nach
